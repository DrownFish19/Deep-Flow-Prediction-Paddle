...
class PatchSplitting(nn.Layer):
    ...
    def __init__(self, input_resolution, dim, norm_layer=nn.LayerNorm):
        ...
        self.input_resolution = input_resolution
        self.dim = dim
        self.upsample = nn.Linear(dim, 2 * dim, bias_attr=True)
    ...
    def forward(self, x):
        ...
        output = paddle.zeros(shape=[B, H * W * 4, C // 2], dtype='float32')
        output = output.reshape([B, H * 2, W * 2, C // 2])  # B H/2*W/2 4*C

        output[:, 0::2, 0::2, :] = x[:, :, :, 0:C // 2]
        output[:, 1::2, 0::2, :] = x[:, :, :, C // 2:C]
        output[:, 0::2, 1::2, :] = x[:, :, :, C:C // 2 * 3]
        output[:, 1::2, 1::2, :] = x[:, :, :, C // 2 * 3:C * 2]
        output = output.reshape([B, H * W * 4, C // 2])
...
class SwinTransformer(nn.Layer):
    ...
    def forward_features(self, x):
        ...
        # x = self.norm(x)  # B L C 此处输出内容转换为 B H W C即可
